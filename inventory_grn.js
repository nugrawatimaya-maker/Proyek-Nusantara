// ==========================================
// INVENTORY GRN LOGIC (Refactored)
// ==========================================

// --- Toast Notification System (Copied from dashboard_kost.js) ---
const Toast = {
    container: null,
    init() {
        if (!this.container) {
    ...
    show(message, type = 'info') {
        ...
        // Style border
        el.classList.add(colorClass.split(' ')[0]);
...
document.addEventListener('DOMContentLoaded', () => {
    if (!window.client) {
        showError("CRITICAL ERROR: Supabase Client not initialized in app_config.js!");
        return;
    }
...
async function switchMode(mode) {
    ...
    // Show Filter Bar ALWAYS (Universal)
...
async function loadOpenPO() {
    ...
    try {
        container.innerHTML = `<div class="text-center p-10 bg-white rounded shadow animate-pulse"><p class="text-gray-500 font-bold">🔄 Mengambil data (${currentMode === 'pending' ? 'Pending' : 'Riwayat'})...</p></div>`;
        ...
        if (currentMode === 'pending') {
            ...
            badge.innerHTML = `⚠️ ${totalPO} PO Menunggu`;
            badge.classList.remove("hidden");
            ...
            if (allStok) {
                allStok.forEach(s => {
                    if (s.nama_barang) {
                        stokMap[s.nama_barang.trim().toLowerCase()] = s.id;
                    }
                });
            }
            ...
            Object.keys(groupedPO).forEach(nomorPO => {
                ...
                items.forEach(po => {
                    ...
                    // FALLBACK: JIKA ID NULL, CARI DI MAP (CASE INSENSITIVE)
                    if (!bId && req?.nama_barang) {
                        const normName = req.nama_barang.trim().toLowerCase();
                        if (stokMap[normName]) {
                            bId = stokMap[normName];
                        }
                    }
            ...
            Object.keys(groupedPO).forEach(nomorPO => {
                ...
                const totalNota = items.reduce((sum, item) => {
                    const qty = item.inventory_req?.jumlah || 0;
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6 opacity-90 hover:opacity-100 transition">
                    <div class="bg-gray-50 px-6 py-3 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-2">
                        <div>
                            ...
                            <p class="text-xs text-gray-600 font-bold mt-1"><i class="fas fa-building mr-1"></i>${items[0].nama_vendor || '-'}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs text-gray-400 font-bold uppercase">Total Nota</p>
                            <p class="text-lg font-bold text-blue-700">Rp ${totalNota.toLocaleString('id-ID')}</p>
                        </div>
                        <div>
                            ${notaButtonHtml}
                        </div>
                ...
                items.forEach(item => {
                    ...
                    const subtotal = (req?.jumlah || 0) * (item.harga_satuan || 0);

                    html += `
                        <div class="flex justify-between items-center py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 px-2 rounded">
                            <div>
                            ...
                            <div class="text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <span class="text-xs font-bold text-gray-600 bg-gray-100 px-2 py-0.5 rounded">${req?.jumlah} ${req?.satuan}</span>
                                    <span class="text-xs text-gray-400">x</span>
                                    <span class="text-xs font-bold text-gray-600">Rp ${(item.harga_satuan || 0).toLocaleString()}</span>
                                </div>
                                <p class="text-sm font-bold text-blue-600 mt-1">Rp ${subtotal.toLocaleString()}</p>
                            </div>
                        </div>
                    `;
                });
        ...
        console.error(err);
        container.innerHTML = `<p class="text-red-500 text-center font-bold">⚠️ Error: ${err.message}</p>`;
    }
}
...
function verifikasiItem(id, namaBarang, groupId) {
    ...
    if (valQty <= 0) return showInfo("Jumlah fisik tidak boleh 0!");
    if (valHarga <= 0) return showInfo("Harga nota wajib diisi!");
...
function recalculateSystemTotal(groupId) {
    // Optional helper function if we made system total dynamic (not implemented in original fully)
    // currently 'verifikasiItem' triggers calculation
}
...
function cekKesesuaian(groupId) {
    ...
    if (!inputNota.value) {
        msgBox.innerHTML = "⚠️ Masukkan total nota fisik";
    ...
    if (selisih === 0) {
        msgBox.innerHTML = "✅ BALANCE / COCOK";
        ...
        msgBox.innerHTML = `❌ SELISIH: Rp ${Math.abs(selisih).toLocaleString('id-ID')} (${ket})`;
...
async function finalUpdate(event, safeID, nomorPO) {
    ...
    if (manualTotal <= 0) {
        ...
        return showError("⚠️ WAJIB ISI TOTAL NOTA DULU SEBELUM MELANJUTKAN.");
    }
    ...
    if (Math.abs(manualTotal - rawValue) > 100) {
        return showError("❌ STOP! Total Nota dan Total Sistem masih selisih.\nMohon cek kembali inputan harga per item.");
    }

    const fileInput = document.getElementById(`nota-${safeID}`);
    if (!fileInput || fileInput.files.length === 0) {
        return showError("❌ STOP: Anda belum mengupload Foto Nota untuk PO ini!");
    }
    ...
    try {
        ...
        for (const row of Array.from(rowsInCard)) {
            ...
            if (resolvedBarangId && resolvedBarangId !== 'null' && resolvedBarangId !== 'undefined') {
                ...
                throw new Error(`Data Barang '${debugName}' tidak ditemukan di Master Stok (ID Null) dan gagal dicocokkan otomatis. \n\nSolusi:\n1. Cek menu Master Stok.\n2. Pastikan nama barang '${debugName}' ada (Ejaan harus mirip).\n3. Hubungi IT jika nama sudah sama tapi error.`);
            }
            ...
            if (rpcError) {
                ...
                const { error: stockErr } = await client.rpc('increment_stock', { x_id: Number(resolvedBarangId), x_qty: qty });
                // Jika increment_stock jg gak ada, coba direct update (unsafe race condition but better than fail)
                if (stockErr) {
                    const { data: currStok } = await client.from('master_stok').select('total_stok').eq('id', resolvedBarangId).single();
                    const newStok = (currStok?.total_stok || 0) + qty;
                    await client.from('master_stok').update({ total_stok: newStok, harga_estimasi: price }).eq('id', resolvedBarangId);
                }
        ...
        if (isTestMode) {
            ...
            showSuccess("💾 Data Test disimpan permanen.");
        } else {
            showSuccess("✅ SUKSES! Stok telah bertambah dan tagihan tercatat.");
        }
        ...
        setTimeout(() => window.location.reload(), 3000); // Wait a bit before reload code
    }
}

async function batalSatuPO(nomorPO) {
    const alasan = prompt(`⚠️ KONFIRMASI PEMBATALAN\n\nAnda akan membatalkan/menolak PO: ${nomorPO}.\nPO ini akan hilang dari halaman GRN.\n\nMasukkan alasan:`);
    if (alasan === null) return;
    if (!alasan.trim()) return showError("❌ Alasan wajib diisi!");
    ...
    try {
        ...
        if (error) throw error;
        showSuccess("✅ STATUS PO BERHASIL DIBATALKAN.");
        loadOpenPO(); // Refresh

    } catch (e) {
        console.error(e);
        showError("❌ Gagal membatalkan PO: " + e.message);
    }
}

// --- NEW: Generate PDF Report for GRN ---
async function downloadGRNReport() {
    ...
    try {
        // 1. Determine Fetch Logic (Same as loadOpenPO but without UI rendering)
        let query = client
            .from('purchase_order')
            .select(`*, inventory_req!fk_req_id ( nama_barang, satuan, jumlah )`)
            .ilike('nama_proyek', activeProject)
        ...
        if (currentMode === 'pending') {
            ...
            query = query.in('status', ['APPROVED', 'DITERIMA', 'SELESAI', 'LUNAS']);
        }
        ...
        if (error) throw error;
        if (!data || data.length === 0) throw new Error("Tidak ada data untuk dicetak.");

        // 2. Filter Logic (Debt Check)
        const { data: debtData } = await client.from('ap_monitoring_vw').select('nomor_po').limit(2000);
        const debtSet = new Set((debtData || []).map(d => d.nomor_po));

        let finalData = [];
        if (currentMode === 'pending') {
            finalData = data.filter(item => !debtSet.has(item.nomor_po));
        } else {
            finalData = data.filter(item => {
                const isDone = ['DITERIMA', 'SELESAI', 'LUNAS'].includes(item.status);
                const isHiddenDone = item.status === 'APPROVED' && debtSet.has(item.nomor_po);
                return isDone || isHiddenDone;
            });
        }

        if (finalData.length === 0) throw new Error("Data setelah filter kosong.");
        ...
        const modeLabel = currentMode === 'pending' ? 'PENDING (Belum GRN)' : 'RIWAYAT (Sudah GRN)';
        doc.text(`LAPORAN PENERIMAAN BARANG (GRN) - ${modeLabel}`, 14, 15);

        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.text(`Proyek: ${activeProject}`, 14, 22);
        doc.text(`Periode: ${startDate || 'Awal'} s/d ${endDate || 'Hari Ini'}`, 14, 27);
        doc.text(`Dicetak: ${new Date().toLocaleString('id-ID')}`, 200, 27);

        // Table Data
        // Group by PO to count items? Or flat list? 
        // Let's do Flat List of POs with Item Summary
        const grouped = finalData.reduce((acc, item) => {
            const no = item.nomor_po;
            if (!acc[no]) acc[no] = { ...item, count: 0, total_val: 0 };
            acc[no].count++;
            acc[no].total_val += (item.total_harga || 0);
            return acc;
        }, {});
        const listPO = Object.values(grouped);

        const tableBody = listPO.map((po, i) => [
            i + 1,
            po.nomor_po,
            new Date(po.created_at).toLocaleDateString('id-ID'),
            po.nama_vendor || '-',
            `${po.count} Item`,
            // Helper fmt might be missing, using toLocaleString
            (po.total_val || 0).toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }),
            currentMode === 'pending' ? 'Menunggu GRN' : 'Sudah Diterima'
        ]);

        doc.autoTable({
            startY: 35,
            head: [['No', 'Nomor PO', 'Tanggal', 'Vendor', 'Jml Item', 'Total Nilai', 'Status']],
            body: tableBody,
            theme: 'grid',
            headStyles: { fillColor: currentMode === 'pending' ? [22, 160, 133] : [41, 128, 185] },
            columnStyles: {
        ...
        doc.save(`Laporan_GRN_${currentMode}_${new Date().toISOString().split('T')[0]}.pdf`);

    } catch (e) {
        showError("Gagal mencetak PDF: " + e.message);
        console.error(e);
    } finally {
...
async function handleReupload(input, nomorPO) {
    ...
    try {
        ...
        showSuccess("✅ Upload Berhasil! Nota telah diperbarui.");
